...
#define SPHTMD(Latitude)                                                       \
  ((double)(TranMerc_ap * Latitude - TranMerc_bp * sin(2.e0 * Latitude) +      \
            TranMerc_cp * sin(4.e0 * Latitude) -                               \
            TranMerc_dp * sin(6.e0 * Latitude) +                               \
            TranMerc_ep * sin(8.e0 * Latitude)))

#define SPHSN(Latitude)                                                        \
  ((double)(TranMerc_a / sqrt(1.e0 - TranMerc_es * pow(sin(Latitude), 2))))

#define SPHSR(Latitude)                                                        \
  ((double)(TranMerc_a * (1.e0 - TranMerc_es) / pow(DENOM(Latitude), 3)))
...
static double TranMerc_es =
    0.0066943799901413800; /* Eccentricity (0.08181919084262188000) squared */
static double TranMerc_ebs =
    0.0067394967565869; /* Second Eccentricity squared */
...
{ /* BEGIN Set_Tranverse_Mercator_Parameters */
   ...
   * (input) Origin_Latitude   : Latitude in radians at the origin of the
   * (input) projection Central_Meridian  : Longitude in radians at the center
   * of the  (input) projection False_Easting     : Easting/X at the center of
   * the projection  (input) False_Northing    : Northing/Y at the center of the
   * projection (input) Scale_Factor      : Projection scale factor (input)
   */
  ...
  if ((Origin_Latitude < -PI_OVER_2) ||
      (Origin_Latitude > PI_OVER_2)) { /* origin latitude out of range */
    Error_Code |= TRANMERC_ORIGIN_LAT_ERROR;
  }
  if ((Central_Meridian < -PI) ||
      (Central_Meridian > (2 * PI))) { /* origin longitude out of range */
  ...
  if (!Error_Code) { /* no errors */
    ...
    TranMerc_ap = TranMerc_a * (1.e0 - tn + 5.e0 * (tn2 - tn3) / 4.e0 +
                                81.e0 * (tn4 - tn5) / 64.e0);
    TranMerc_bp = 3.e0 * TranMerc_a *
                  (tn - tn2 + 7.e0 * (tn3 - tn4) / 8.e0 + 55.e0 * tn5 / 64.e0) /
                  2.e0;
    TranMerc_cp =
        15.e0 * TranMerc_a * (tn2 - tn3 + 3.e0 * (tn4 - tn5) / 4.e0) / 16.0;
    TranMerc_dp =
        35.e0 * TranMerc_a * (tn3 - tn4 + 11.e0 * tn5 / 16.e0) / 48.e0;
    TranMerc_ep = 315.e0 * TranMerc_a * (tn4 - tn5) / 512.e0;
    Convert_Geodetic_To_Transverse_Mercator(
        MAX_LAT, MAX_DELTA_LONG + Central_Meridian, &TranMerc_Delta_Easting,
        &TranMerc_Delta_Northing);
    Convert_Geodetic_To_Transverse_Mercator(
        0, MAX_DELTA_LONG + Central_Meridian, &TranMerc_Delta_Easting,
        &dummy_northing);
    TranMerc_Delta_Northing++;
...
{ /* BEGIN Get_Tranverse_Mercator_Parameters  */
   ...
   *    f                 : Flattening of ellipsoid
   * (output) Origin_Latitude   : Latitude in radians at the origin of the
   * (output) projection Central_Meridian  : Longitude in radians at the center
   * of the  (output) projection False_Easting     : Easting/X at the center of
   * the projection  (output) False_Northing    : Northing/Y at the center of
   * the projection (output) Scale_Factor      : Projection scale factor
   * (output)
   */
...
{ /* BEGIN Convert_Geodetic_To_Transverse_Mercator */
  ...
  if (!Error_Code) { /* no errors */
    ...
    if (fabs(dlam) >
        (9.0 * PI / 180)) { /* Distortion will result if Longitude is more than
                               9 degrees from the Central Meridian */
    ...
    t4 = sn * s * c5 * TranMerc_Scale_Factor *
         (61.e0 - 58.e0 * tan2 + tan4 + 270.e0 * eta - 330.e0 * tan2 * eta +
          445.e0 * eta2 + 324.e0 * eta3 - 680.e0 * tan2 * eta2 + 88.e0 * eta4 -
    ...
    *Northing = TranMerc_False_Northing + t1 + pow(dlam, 2.e0) * t2 +
    ...
    t8 = sn * c5 * TranMerc_Scale_Factor *
         (5.e0 - 18.e0 * tan2 + tan4 + 14.e0 * eta - 58.e0 * tan2 * eta +
          13.e0 * eta2 + 4.e0 * eta3 - 64.e0 * tan2 * eta2 -
          24.e0 * tan2 * eta3) /
         120.e0;
...
long Convert_Transverse_Mercator_To_Geodetic(
    double Easting, double Northing, double *Latitude,
  ...
  if (!Error_Code) {
    ...
    t11 = t *
          (5.e0 + 3.e0 * tan2 + eta - 4.e0 * pow(eta, 2) - 9.e0 * tan2 * eta) /
          (24.e0 * sr * pow(sn, 3) * pow(TranMerc_Scale_Factor, 4));
    t12 = t *
          (61.e0 + 90.e0 * tan2 + 46.e0 * eta + 45.E0 * tan4 -
           252.e0 * tan2 * eta - 3.e0 * eta2 + 100.e0 * eta3 -
           66.e0 * tan2 * eta2 - 90.e0 * tan4 * eta + 88.e0 * eta4 +
           225.e0 * tan4 * eta2 + 84.e0 * tan2 * eta3 - 192.e0 * tan2 * eta4) /
          (720.e0 * sr * pow(sn, 5) * pow(TranMerc_Scale_Factor, 6));
    t13 = t *
          (1385.e0 + 3633.e0 * tan2 + 4095.e0 * tan4 + 1575.e0 * pow(t, 6)) /
    ...
    *Latitude = ftphi - pow(de, 2) * t10 + pow(de, 4) * t11 - pow(de, 6) * t12 +
    ...
    t15 = (1.e0 + 2.e0 * tan2 + eta) /
          (6.e0 * pow(sn, 3) * c * pow(TranMerc_Scale_Factor, 3));

    t16 = (5.e0 + 6.e0 * eta + 28.e0 * tan2 - 3.e0 * eta2 + 8.e0 * tan2 * eta +
           24.e0 * tan4 - 4.e0 * eta3 + 4.e0 * tan2 * eta2 +
           24.e0 * tan2 * eta3) /
    ...
    if (fabs(dlam) >
        (9.0 * PI / 180) *
            cos(*Latitude)) { /* Distortion will result if Longitude
                                 is more than 9 degrees from the
