...
long Set_UTM_Parameters(double a, double f, long override) {
   ...
   *
   *    a                 : Semi-major axis of ellipsoid, in meters (input) f :
   * Flattening of ellipsoid						(input)
   *    override          : UTM override zone, zero indicates no override
   * (input)
   */
...
void Get_UTM_Parameters(double *a, double *f, long *override) {
   ...
   *
   *    a                 : Semi-major axis of ellipsoid, in meters (output) f
   * : Flattening of ellipsoid (output) override          : UTM override zone,
   * zero indicates no override (output)
   */
  ...
  if (!Error_Code) { /* no errors */
    ...
    if (UTM_Override) {
      if ((temp_zone == 1) && (UTM_Override == 60))
        ...
        temp_zone = UTM_Override;
      else if ((Lat_Degrees > 71) && (Long_Degrees > -1) &&
               (Long_Degrees < 42)) {
        if (((temp_zone - 2) <= UTM_Override) &&
            (UTM_Override <= (temp_zone + 2)))
          temp_zone = UTM_Override;
        else
          Error_Code = UTM_ZONE_OVERRIDE_ERROR;
...
long Convert_UTM_To_Geodetic(long Zone, char Hemisphere, double Easting,
  ...
  if (!Error_Code) { /* no errors */
    ...
    tm_error_code = Convert_Transverse_Mercator_To_Geodetic(
