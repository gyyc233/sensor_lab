...
#include "preintegration_lio.h"

namespace sad {
LIO_PreIntegration::LIO_PreIntegration(Options options)
    : options_(options), preinteg_(new IMUPreintegration()) {
  ...
  // gyro 与 acce 各有三维故信息矩阵为6*6
  options_.ndt_info_.diagonal() << 1.0 / ga2, 1.0 / ga2, 1.0 / ga2, 1.0 / gp2,
      1.0 / gp2, 1.0 / gp2;
...
bool LIO_PreIntegration::Init(const std::string &config_yaml) {
...
void LIO_PreIntegration::ProcessMeasurements(const MeasureGroup &meas) {
...
bool LIO_PreIntegration::LoadFromYAML(const std::string &yaml_file) {
...
void LIO_PreIntegration::Align() {
  ...
  current_nav_state_ =
      preinteg_->Predict(last_nav_state_, imu_init_.GetGravity());
  ...
  if (delta_pose.translation().norm() > 0.3 ||
      delta_pose.so3().log().norm() > math::deg2rad(5.0)) {
    // 将地图合入NDT中
...
void LIO_PreIntegration::TryInitIMU() {
...
void LIO_PreIntegration::Undistort() {
...
void LIO_PreIntegration::Predict() {
  imu_states_.clear();
  ...
  for (auto &imu : measures_.imu_) {
    ...
    last_imu_ = imu;
    imu_states_.emplace_back(
        preinteg_->Predict(last_nav_state_, imu_init_.GetGravity()));
  }
}

void LIO_PreIntegration::PCLCallBack(
    const sensor_msgs::PointCloud2::ConstPtr &msg) {
  sync_->ProcessCloud(msg);
}

void LIO_PreIntegration::LivoxPCLCallBack(
    const livox_ros_driver::CustomMsg::ConstPtr &msg) {
  sync_->ProcessCloud(msg);
}

void LIO_PreIntegration::IMUCallBack(IMUPtr msg_in) {
  sync_->ProcessIMU(msg_in);
}

void LIO_PreIntegration::Finish() { LOG(INFO) << "finish done"; }

void LIO_PreIntegration::Optimize() {
  // 调用g2o求解优化问题
  ...
  /// NOTE
  /// 这些东西是对参数非常敏感的。相差几个数量级的话，容易出现优化不动的情况
  /// 看到上面这句有点慌了 阿米诺斯
  ...
  if (options_.verbose_) {
    ...
    LOG(INFO) << "preinteg chi2: " << edge_inertial->chi2()
              << ", err: " << edge_inertial->error().transpose();
    LOG(INFO) << "prior chi2: " << edge_prior->chi2()
              << ", err: " << edge_prior->error().transpose();
    LOG(INFO) << "ndt: " << edge_ndt->chi2() << "/"
              << edge_ndt->error().transpose();
  }
...
void LIO_PreIntegration::NormalizeVelocity() {
  /// 限制v的变化
  /// 一般是-y 方向速度
