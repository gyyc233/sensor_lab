...
namespace sad {
IncrementalNdt3D::IncrementalNdt3D() {
  options_.inv_voxel_size_ = 1.0 / options_.voxel_size_;
  generateNearbyGrids();
}

IncrementalNdt3D::IncrementalNdt3D(Options options) : options_(options) {
  options_.inv_voxel_size_ = 1.0 / options_.voxel_size_;
  generateNearbyGrids();
}
...
void IncrementalNdt3D::addCloud(CloudPtr cloud_world) {
  std::set<KeyType, less_vec<3>> active_voxels; // 记录哪些voxel被更新
  for (const auto &p : cloud_world->points) {
    auto pt = ToVec3d(p);
    LOG(INFO) << "cloud_world point: " << pt;
    auto key = CastToInt(Vec3d(pt * options_.inv_voxel_size_));
    LOG(INFO) << "belong to voxel id: " << key;

    auto find_voxel_id = grids_.find(key);
...
void IncrementalNdt3D::setSource(CloudPtr source) { source_ = source; }
...
void IncrementalNdt3D::updateVoxel(VoxelData &v) {
  ...
  if (flag_first_scan_) {
    if (v.pts_.size() > 1) {
      ...
      v.info_ =
          (v.sigma_ + Mat3d::Identity() * 1e-3).inverse(); // 避免信息矩阵出nan
    } else {
      v.mu_ = v.pts_[0];
  ...
  if (!v.ndt_estimated_ && v.pts_.size() > options_.min_pts_in_voxel_) {
    ...
    math::ComputeMeanAndCov(v.pts_, cur_mu, cur_var,
                            [this](const Vec3d &p) { return p; });
    math::UpdateMeanAndCov(v.num_pts_, (int)v.pts_.size(), v.mu_, v.sigma_,
...
bool IncrementalNdt3D::alignNdt(SE3 &init_pose) {
  LOG(INFO) << "aligning with incremental ndt, source pts: " << source_->size()
            << ", grids number: " << grids_.size();
...
void IncrementalNdt3D::buildNDTEdges(VertexPose *v,
