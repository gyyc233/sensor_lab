...
void LoamLikeOdom::processPointCloud(FullCloudPtr full_cloud) {
  ...
  if (current_edge->size() < options_.min_edge_pts_ ||
    ...
    LOG(ERROR) << "not enough edge/surf pts: " << current_edge->size() << ","
               << current_surf->size();
    return;
  }

  LOG(INFO) << "edge: " << current_edge->size()
            << ", surf: " << current_surf->size();
  ...
  pcl::transformPointCloud(*ConvertToCloud<FullPointType>(full_cloud),
                           *scan_world, pose.matrix());
...
SE3 LoamLikeOdom::alignWithLocalMap(CloudPtr edge, CloudPtr surf) {
  ...
  for (int iter = 0; iter < options_.max_iteration_; ++iter) {
    ...
    std::vector<Eigen::Matrix<double, 1, 6>> jacob_surf(
        surf_size); // 点面的残差是1维的
    std::vector<double> errors_surf(surf_size);

    std::vector<bool> effect_edge(edge_size, false);
    std::vector<Eigen::Matrix<double, 3, 6>> jacob_edge(
        edge_size); // 点线的残差是3维的
