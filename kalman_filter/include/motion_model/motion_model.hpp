...
template <int32_t DIM_X> class MotionModel {
public:
  ...
  virtual Vector<DIM_X>
  f(Vector<DIM_X> const &vecX,
  ...
  virtual Matrix<DIM_X, DIM_X>
  getProcessNoiseCov(Vector<DIM_X> const &vecX) const = 0;
  ...
  virtual Matrix<DIM_X, DIM_X>
  getJacobianFk(Vector<DIM_X> const &vecX) const = 0;
};
...
template <int32_t DIM_X, int32_t DIM_U> class MotionModelExtInput {
public:
  /// @brief Prediction motion model function that propagate the previous state
  /// to next state in time.
  /// @param vecX State space vector \vec{x}
  ...
  /// @return Predicted/ propagated state space vector
  virtual Vector<DIM_X>
  f(Vector<DIM_X> const &vecX, Vector<DIM_U> const &vecU,
    Vector<DIM_X> const &vecQ = Vector<DIM_X>::Zero()) const = 0;
  ...
  /// @return The process noise covariance Q
  virtual Matrix<DIM_X, DIM_X>
  getProcessNoiseCov(Vector<DIM_X> const &vecX,
                     Vector<DIM_U> const &vecU) const = 0;
  ...
  virtual Matrix<DIM_X, DIM_X>
  getInputNoiseCov(Vector<DIM_X> const &vecX,
                   Vector<DIM_U> const &vecU) const = 0;
  ...
  /// @return The jacobians of the state transition model.
  virtual Matrix<DIM_X, DIM_X>
  getJacobianFk(Vector<DIM_X> const &vecX, Vector<DIM_U> const &vecU) const = 0;
  ...
  virtual Matrix<DIM_X, DIM_U>
  getJacobianBk(Vector<DIM_X> const &vecX, Vector<DIM_U> const &vecU) const = 0;
};
} // namespace motionmodel
} // namespace kf
