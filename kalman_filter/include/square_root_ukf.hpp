...
public:
  ...
  Matrix<DIM_X, DIM_X> &matP() override {
  ...
  void initialize(const Vector<DIM_X> &vecX, const Matrix<DIM_X, DIM_X> &matP,
                  const Matrix<DIM_X, DIM_X> &matQ,
                  const Matrix<DIM_Z, DIM_Z> &matR) {
    m_vecX = vecX;

    // 分别对P Q R 做cholesky因式分解，求平方根矩阵
    {
  ...
  void predict_SRUKF(PredictionModelCallback predictionModelFunc) {
    const float32_t kappa{static_cast<float32_t>(3 - DIM_X)};

    // x_sigmas = self.sigma_points(self.x, self.S)
    Matrix<DIM_X, SIGMA_DIM> matSigmaX{
        calculateSigmaPoints(m_vecX, m_matSk, kappa)};
  ...
  void correct_SRUKF(MeasurementModelCallback measurementModelFunc,
...
private:
  ...
  Matrix<DIM_X, DIM_X> m_matSk{
      Matrix<DIM_X,
             DIM_X>::Zero()}; /// @brief augmented state covariance (incl.
                              /// process and measurement noise covariances)
  Matrix<DIM_X, DIM_X> m_matRv{
      Matrix<DIM_X,
             DIM_X>::Zero()}; /// @brief augmented state covariance (incl.
                              /// process and measurement noise covariances)
  Matrix<DIM_Z, DIM_Z> m_matRn{
      Matrix<DIM_Z,
             DIM_Z>::Zero()}; /// @brief augmented state covariance (incl.
                              /// process and measurement noise covariances)
  ...
  calculateSigmaPoints(const Vector<DIM_X> &vecXk,
                       const Matrix<DIM_X, DIM_X> &matSk,
                       const float32_t kappa) {
    const Matrix<DIM_X, DIM_X> matS{
        matSk * std::sqrt(DIM_X + kappa)}; // sqrt(n + \kappa) * Sk
    ...
    for (int32_t i{0}; i < DIM_X; ++i) {
      ...
      const Vector<DIM_X> vecShiftTerm{
          util::getColumnAt<DIM_X, DIM_X>(i, matS)};

      util::addColumnFrom<DIM_X, SIGMA_DIM>(
          IDX_1, sigmaX,
          vecShiftTerm); // X_i     = \bar{x} + sqrt(n + \kappa) * Sk
      util::subColumnFrom<DIM_X, SIGMA_DIM>(
          IDX_2, sigmaX,
  ...
  void calculateWeightedMean(const Matrix<DIM, SIGMA_DIM> &sigmaX,
                             Vector<DIM> &vecX) {
    ...
    for (int32_t i{1}; i < SIGMA_DIM; ++i) {
      vecX += m_weighti * util::getColumnAt<DIM, SIGMA_DIM>(
  ...
  template <int32_t DIM>
  Matrix<DIM, DIM>
  updatePredictedCovariance(const Matrix<DIM, SIGMA_DIM> &matSigmaX,
                            const Vector<DIM> &meanX,
    ...
    // 创建sigma 和　过程噪声协方差矩阵　联合矩阵，对这个联合矩阵做QR分解 A=QR
    // Q是正交矩阵 R 上三角矩阵
    ...
    qrMatrixUpper =
        util::getUpperTriangulerView<DIM_ROW, DIM_COL>(qrMatrixUpper);
    ...
    Matrix<DIM, DIM> matR{
        util::getBlock<DIM_ROW, DIM_COL, DIM, DIM>(qrMatrixUpper, 0, 0)};
  ...
  buildCompoundMatrix(const Matrix<DIM_X, SIGMA_DIM> &matSigmaX,
