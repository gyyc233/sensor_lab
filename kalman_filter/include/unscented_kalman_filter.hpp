...
public:
  ...
  void predictUKF(PredictionModelCallback predictionModelFunc) {
    ...
    Matrix<DIM_V, SIGMA_DIM> sigmaXv{
        matSigmaXa.block(DIM_X, 0, DIM_V, SIGMA_DIM)};

    // y_sigmas = np.zeros((self.dim_x, self.n_sigma))
    // for i in range(self.n_sigma):
    //     y_sigmas[:, i] = f(xx_sigmas[:, i], xv_sigmas[:, i])
    for (int32_t i{0}; i < SIGMA_DIM; ++i) {
      const Vector<DIM_X> sigmaXxi{
          util::getColumnAt<DIM_X, SIGMA_DIM>(i, sigmaXx)};
      const Vector<DIM_V> sigmaXvi{
          util::getColumnAt<DIM_V, SIGMA_DIM>(i, sigmaXv)};

      const Vector<DIM_X> Yi{
          predictionModelFunc(sigmaXxi, sigmaXvi)}; // y = f(x)
  ...
  void correctUKF(MeasurementModelCallback measurementModelFunc,
    ...
    Matrix<DIM_N, SIGMA_DIM> sigmaXn{
        matSigmaXa.block(DIM_X + DIM_V, 0, DIM_N, SIGMA_DIM)};
    ...
    for (int32_t i{0}; i < SIGMA_DIM; ++i) {
      const Vector<DIM_X> sigmaXxi{
          util::getColumnAt<DIM_X, SIGMA_DIM>(i, sigmaXx)};
      const Vector<DIM_N> sigmaXni{
          util::getColumnAt<DIM_N, SIGMA_DIM>(i, sigmaXn)};

      const Vector<DIM_Z> Yi{
          measurementModelFunc(sigmaXxi, sigmaXni)}; // y = f(x)
...
private:
  ...
  float32_t
      m_weighti; /// @brief unscented transform weight i for none mean samples

  Vector<DIM_A> m_vecXa{
      Vector<DIM_A>::Zero()}; /// @brief augmented state vector (incl. process
                              /// and measurement noise means)
  Matrix<DIM_A, DIM_A> m_matPa{
      Matrix<DIM_A,
             DIM_A>::Zero()}; /// @brief augmented state covariance (incl.
  ...
  calculateSigmaPoints(const Vector<DIM_A> &vecXa,
                       const Matrix<DIM_A, DIM_A> &matPa,
                       const float32_t kappa) {
    const float32_t scalarMultiplier{
        std::sqrt(DIM_A + kappa)}; // sqrt(n + \kappa)
    ...
    for (int32_t i{0}; i < DIM_A; ++i) {
      ...
      const Vector<DIM_A> vecShiftTerm{
          util::getColumnAt<DIM_A, DIM_A>(i, matSa)};

      util::addColumnFrom<DIM_A, SIGMA_DIM>(
          IDX_1, sigmaXa, vecShiftTerm); // X_i^a     = \bar{xa} + sqrt( (n^a +
                                         // \kappa) * P^{a} )
      util::subColumnFrom<DIM_A, SIGMA_DIM>(
  ...
  void
  calculateWeightedMeanAndCovariance(const Matrix<STATE_DIM, SIGMA_DIM> &sigmaX,
                                     Vector<STATE_DIM> &vecX,
    ...
    for (int32_t i{1}; i < SIGMA_DIM; ++i) {
      vecX += m_weighti * util::getColumnAt<STATE_DIM, SIGMA_DIM>(
                              i, sigmaX); // y += W[0, i] Y[:, i]
    }

    // 2. calculate covariance: P_{yy} = \sum_{i_0}^{2n} W[0, i] (Y[:, i] -
    // \bar{y}) (Y[:, i] - \bar{y})^T
    Vector<STATE_DIM> devXi{util::getColumnAt<STATE_DIM, SIGMA_DIM>(0, sigmaX) -
                            vecX}; // Y[:, 0] - \bar{ y }
    matPxx =
        m_weight0 * devXi * devXi.transpose(); // P_0 = W[0, 0] (Y[:, 0] -
                                               // \bar{y}) (Y[:, 0] - \bar{y})^T

    for (int32_t i{1}; i < SIGMA_DIM; ++i) {
      devXi = util::getColumnAt<STATE_DIM, SIGMA_DIM>(i, sigmaX) -
              vecX; // Y[:, i] - \bar{y}

      const Matrix<STATE_DIM, STATE_DIM> Pi{
          m_weighti * devXi *
  ...
  Matrix<DIM_X, DIM_Z> calculateCrossCorrelation(
    ...
    Vector<DIM_X> devXi{util::getColumnAt<DIM_X, SIGMA_DIM>(0, sigmaX) -
                        vecX}; // X[:, 0] - \bar{ x }
    Vector<DIM_Z> devYi{util::getColumnAt<DIM_Z, SIGMA_DIM>(0, sigmaY) -
                        vecY}; // Y[:, 0] - \bar{ y }
    ...
    for (int32_t i{1}; i < SIGMA_DIM; ++i) {
      devXi = util::getColumnAt<DIM_X, SIGMA_DIM>(i, sigmaX) -
              vecX; // X[:, i] - \bar{x}
      devYi = util::getColumnAt<DIM_Z, SIGMA_DIM>(i, sigmaY) -
              vecY; // Y[:, i] - \bar{y}

      matPxy += m_weighti *
                (devXi * devYi.transpose()); // y += W[0, i] (Y[:, i] -
                                             // \bar{y}) (Y[:, i] - \bar{y})^T
    }
