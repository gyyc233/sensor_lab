...
bool ConvertGps2UTM(GNSS &gps_msg, const Vec2d &antenna_pos,
  ...
  /// TWG 转到 TWB
  SO3 tmp_rot_z =
      SO3::exp(Eigen::Vector3d(0, 0, antenna_angle * math::kDEG2RAD));
  SE3 TBG(tmp_rot_z, Vec3d(antenna_pos[0], antenna_pos[1], 0));
  SE3 TGB = TBG.inverse();
  ...
  tmp_rot_z = SO3::exp(Eigen::Vector3d(0, 0, heading));
  SE3 TWG(tmp_rot_z, Vec3d(x, y, z));
  SE3 TWB = TWG * TGB;
