...
namespace sad {
EdgeInertial::EdgeInertial(std::shared_ptr<IMUPreintegration> preinteg,
                           const Vec3d &gravity, double weight)
    : preint_(preinteg), dt_(preinteg->dt_) {
  // 定义6元边
  resize(6); // 6个关联顶点
  grav_ = gravity;
  // 设置信息矩阵是n阶方阵，也是协方差矩阵之逆；
  // 某次观测非常准确，那么协方差矩阵就会"小"，而信息矩阵就会"大"
  // 信息矩阵是加权后的欧氏距离，也叫做马氏距离
  // 信息矩阵越小，对应协方差越大，说明距离真值的误差越大，在优化时就要给予较小权重
...
void EdgeInertial::computeError() {
  ...
  const Vec3d ev =
      RiT * (v2->estimate() - v1->estimate() - grav_ * dt_) - dv; // 速度残差
...
void EdgeInertial::linearizeOplus() {
  ...
  const Mat3d invJr = preint_->jr_inv_test(er); // SO3右扰动
  ...
  _jacobianOplus[2].block<3, 3>(0, 0) =
      -invJr * eR.inverse().matrix() *
      preint_->jr_inv_test((dR_dbg * dbg).eval()) * dR_dbg;
  // dv/dbg1
