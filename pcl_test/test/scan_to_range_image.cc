...
DEFINE_string(pcd_path, "./data/pcd/scan_example.pcd", "点云文件路径");
...
void GenerateRangeImage(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud) {
  int image_cols =
      int(360 / FLAGS_azimuth_resolution_deg); // 水平为360度，按分辨率切分即可
  ...
  double ele_resolution =
      FLAGS_elevation_range * 2 / FLAGS_elevation_rows; // elevation分辨率
  ...
  for (const auto &pt : cloud->points) {
    ...
    int y =
        int((elevation + FLAGS_elevation_range) / ele_resolution + 0.5); // 列
