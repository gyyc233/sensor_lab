...
void OccupancyMap::addLidarFrame(std::shared_ptr<Frame> frame,
  ...
  SE2 pose_in_submap = pose_.inverse() * frame->pose_; // frame->pose_ 去中心化
  ...
  // 函数指针法
  // bool testLess(const Vec2i& v1, const Vec2i& v2) {
  //  return v1[0] < v2[0] || (v1[0] == v2[0] && v1[1] < v2[1]);
  //}
  // set<Vec2i, bool(*)(const Vec2i& v1, const Vec2i& v2)> endpoints(testLess);
  // (TODO: 猜测是eigen vector转matrix)
  ...
  if (method == GridMethod::MODEL_POINTS) {
    // 遍历模板，生成白色点
    std::for_each(model_.begin(), model_.end(), [&](const Model2DPoint &pt) {
    ...
    std::for_each(
        ...
        [this, &start](const Vec2i &pt) { bresenhamFilling(start, pt); });
  }
