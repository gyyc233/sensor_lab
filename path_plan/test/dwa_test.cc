 ...
 * @param width
 */
void plotArrow(double x, double y, double yaw, double len = 0.5,
               double width = 0.1) {
...
int main() {
  VectorXd state(5);
  state << 0.0, 0.0, PI / 8.0, 0.0,
      0.0;               //[x(m), y(m), yaw(rad), v(m/s), omega(rad/s)]
  ...
  //初始化
  DWA dwa(dt, v_min, v_max, w_min, w_max, predict_time, a_vmax, a_wmax,
          v_sample, w_sample, alpha, beta, gamma, radius, judge_distance);
