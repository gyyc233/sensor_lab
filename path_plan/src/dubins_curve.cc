...
double Dubins::PI2PI(double angle) { return fmod(angle + PI, 2 * PI) - PI; }
...
Dubins::interpolate(double ind, double length, char m, double max_curvature,
...
vector<vector<double>> Dubins::generate_local_course(double total_length,
  ...
  for (int i = 0; i < modes.size(); i++) {
    ...
    while (abs(pd) <= abs(lengths[i])) {
      ...
      vector<vector<double>> inter =
          interpolate(ind, pd, modes[i], max_curvature, origin_x, origin_y,
    ...
    ind += 1;
    vector<vector<double>> inter =
        interpolate(ind, lengths[i], modes[i], max_curvature, origin_x,
                    origin_y, origin_yaw, p_x, p_y, p_yaw, directions);
    p_x = inter[0], p_y = inter[1], p_yaw = inter[2], directions = inter[3];
  }
  if (p_x.size() <= 1) {
    cout << "no path" << endl;
...
Dubins::ResultDubins
Dubins::dubins_path_planning_from_origin(Vector3d goal, double curvature,
                                         double step_size) {
  double dx = goal[0];
  double dy = goal[1];
  ...
  vector<vector<double>> glc =
      generate_local_course(lengths[0] + lengths[1] + lengths[2], lengths,
                            best_mode, curvature, step_size);

  ResultDubins rd;
  rd.p_x = glc[0], rd.p_y = glc[1], rd.p_yaw = glc[2], rd.directions = glc[3];
  rd.mode = best_mode;
...
Dubins::ResultDubins Dubins::dubins_path_planning(Vector3d start, Vector3d goal,
  ...
  rot =
      AngleAxisd(-start[2], Eigen::Vector3d::UnitZ()); //从欧拉角中获取旋转矩阵
  assert(rd.p_x.size() == rd.p_y.size() &&
         rd.p_y.size() == rd.p_yaw.size()); //确保维度一致
  int sz = rd.p_x.size();
  MatrixXd pxy(sz, 2);
