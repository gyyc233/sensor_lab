...
RRTConnect::RRTConnect(const vector<vector<double>> &obstacleList,
    ...
    : obstacle_list(obstacleList), rand_area(randArea), play_area(playArea),
      robot_radius(robotRadius), expand_dis(expandDis),
 ...
 * @return
 */
vector<double> RRTConnect::calDistanceAngle(RRTConnect::Node *from_node,
                                            RRTConnect::Node *to_node) {
...
//            if (pow(obs[0] - x, 2) + pow(obs[1] - y, 2) <= pow(obs[2] +
//            robot_radius, 2))return false;//collision
//        }
//        //double x = node->x,y=node->y;
//        //if(pow(obs[0]-x,2)+pow(obs[1]-y,2)<=pow(obs[2]+robot_radius,2))return
//        false;//collision
//    }
//    return true;//safe
//}
...
/**
 * Determine 判断是否有障碍物 (using),
 * 使用线段与圆形的相交性方法来实现(general),
 * 对搜索会更加困难，这里便不考虑效率问题了...
 * @param node
 * @return
 */
bool RRTConnect::obstacleFree(Node *node) {
  auto isSegmentIntersectingObstacle = [this](const Point &p1,
                                              const Point &p2) {
    for (const auto &obs : obstacle_list) {
      ...
      const Point closestPointOnLine =
          closestPointOnSegment(p1, p2, obstacleCenter);
      double dist_to_center =
          distanceBetweenPoints(obstacleCenter, closestPointOnLine);
      if (dist_to_center <= obs[2] + robot_radius && dist_to_center >= 0 &&
...
bool RRTConnect::isInsidePlayArea(RRTConnect::Node *node) {
  if (node->x < play_area[0] || node->x > play_area[1] ||
      node->y < play_area[2] || node->y > play_area[3])
    return false;
  return true;
}
 ...
 * @return 最近的节点索引
 */
int RRTConnect::getNearestNodeIndex(vector<Node *> node_list,
                                    RRTConnect::Node *rnd_node) {
  int min_index = -1;
...
RRTConnect::Node *RRTConnect::sampleFree() {
  ...
  if (rand() % (100) > goal_sample_rate) {
    ...
    double min_rand =
        rand() / double(RAND_MAX) * (rand_area[1] - rand_area[0]) +
        rand_area[0];
    double max_rand =
        rand() / double(RAND_MAX) * (rand_area[1] - rand_area[0]) +
        rand_area[0];
...
void RRTConnect::setAnEnd(RRTConnect::Node *anEnd) { end = anEnd; }
 ...
 * @return
 */
RRTConnect::Node *RRTConnect::steer(RRTConnect::Node *from_node,
                                    RRTConnect::Node *to_node,
...
pair<vector<double>, vector<double>> RRTConnect::planning() {
  node_list_1.push_back(
      begin); //将起点作为根节点x_{init}，加入到随机树的节点集合中。
  node_list_2.push_back(
      end); //将终点作为根节点x_{init}，加入到随机树的节点集合中。
    ...
    if (isInsidePlayArea(new_node) && obstacleFree(new_node)) {
      ...
      if (isInsidePlayArea(new_node_2) && obstacleFree(new_node_2)) {
        ...
        while (true) {
          ...
          if (abs(new_node_2->x - new_node->x) < 0.00001 &&
              abs(new_node_2->y - new_node->y) < 0.00001) {
...
void RRTConnect::draw(Node *node1, Node *node2) {
  ...
  plt::plot(vector<double>{play_area[0], play_area[1], play_area[1],
                           play_area[0], play_area[0]},
            vector<double>{play_area[2], play_area[2], play_area[3],
                           play_area[3], play_area[2]},
            "k-");
