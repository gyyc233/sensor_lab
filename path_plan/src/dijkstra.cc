...
Dijkstra::Node::Node(double x, double y, float cost, double parentIndex)
    : x(x), y(y), cost(cost), parent_index(parentIndex) {}

Dijkstra::Dijkstra(double resolution, double robotRadius)
 ...
 * @param oy 障碍物y坐标集合
 */
void Dijkstra::calObstacleMap(const vector<double> &ox,
...
Dijkstra::calFinalPath(Dijkstra::Node *goal_node,
 ...
 * @return 规划后的路径
 */
pair<vector<double>, vector<double>> Dijkstra::planning(vector<double> start,
                                                        vector<double> goal) {
...
void Dijkstra::setSt(const vector<double> &st) { Dijkstra::st = st; }

void Dijkstra::setGo(const vector<double> &go) { Dijkstra::go = go; }

void Dijkstra::setOx(const vector<double> &ox) { Dijkstra::ox = ox; }

void Dijkstra::setOy(const vector<double> &oy) { Dijkstra::oy = oy; }
