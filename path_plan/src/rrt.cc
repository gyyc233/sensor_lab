...
void RRT::setBegin(RRT::Node *begin) { RRT::begin = begin; }

void RRT::setAnEnd(RRT::Node *anEnd) { end = anEnd; }
 ...
 * @return
 */
RRT::Node *RRT::steer(RRT::Node *from_node, RRT::Node *to_node,
...
pair<vector<double>, vector<double>> RRT::planning() {
  ...
  for (int i = 0; i < max_iter; i++) {
    ...
    if (calDistToGoal(node_list[node_list.size() - 1]->x,
                      node_list[node_list.size() - 1]->y) <= expand_dis) {
      Node *final_node =
          steer(node_list[node_list.size() - 1], end, expand_dis);
      if (obstacleFree(final_node)) {
        cout << "reaches the goal!" << endl;
        // return
        // {node_list[node_list.size()-1]->path_x,node_list[node_list.size()-1]->path_y};
