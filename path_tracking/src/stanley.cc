...
Stanley::Stanley(double k) { this->k = k; }

double Stanley::calTargetIndex(vector<double> robot_state,
...
vector<double> Stanley::stanleyControl(vector<double> robot_state,
                                       vector<vector<double>> refer_path) {
  ...
  if ((robot_state[0] - current_ref_point[0]) * psi_t -
          (robot_state[1] - current_ref_point[1]) >
      0) {
    // if((robot_state[1]-current_ref_point[1])*cos(psi_t)-(robot_state[0]-current_ref_point[0])*sin(psi_t)<=0){
    e_y = sqrt(pow(current_ref_point[0] - robot_state[0], 2) +
               pow(current_ref_point[1] - robot_state[1], 2));
  } else {
    e_y = -sqrt(pow(current_ref_point[0] - robot_state[0], 2) +
                pow(current_ref_point[1] - robot_state[1], 2));
  }
