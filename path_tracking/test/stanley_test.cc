...
int main() {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    for (int i = 0; i < 999; i++) {
      refer_path[i][2] = atan2((refer_path[i + 1][1] - refer_path[i][1]),
                               (refer_path[i + 1][0] - refer_path[i][0]));
    }
  ...
  for (int i = 0; i < 600; i++) {
    ...
    //参考博客中的公式
    vector<double> delta_index =
        stanley.stanleyControl(robot_state, refer_path);
    ...
    plt::ylim(-5, 5);
    plt::pause(0.01);
    if (delta_index[1] >= refer_path.size() - 1)
      break;
  }
  // save figure
