...
MyReferencePath::MyReferencePath() {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    refer_path[i][1] =
        2 * sin(refer_path[i][0] / 3.0) + 2.5 * cos(refer_path[i][0] / 2.0);
  ...
  for (int i = 0; i < refer_path.size(); i++) {
    ...
    refer_path[i][3] =
        (ddy * dx - ddx * dy) / pow((dx * dx + dy * dy), 3 / 2); // 曲率k计算
  }
}
 ...
 */
refTraj MyReferencePath::calc_ref_trajectory(std::vector<double> robot_state,
                                             parameters param, double dl) {
  std::vector<double> track_error =
      calcTrackError(robot_state); // e,k,ref_yaw,ind
  double e = track_error[0], k = track_error[1], ref_yaw = track_error[2],
         ind = track_error[3];
  refTraj ref_traj;
