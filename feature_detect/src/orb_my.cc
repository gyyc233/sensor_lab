...
void OrbMy::inputParams(const char *left_img, const char *right_img) {
...
void OrbMy::initialization() {}

void OrbMy::run() {}

void OrbMy::computeORB(const cv::Mat &img,
  ...
  for (auto &kp : key_points) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      for (int k = 0; k < 32; k++) {
        int idx_pq = i * 32 + k;
        cv::Point2f p(ORB_pattern_[idx_pq * 4], ORB_pattern_[idx_pq * 4 + 1]);
        cv::Point2f q(ORB_pattern_[idx_pq * 4 + 2],
                      ORB_pattern_[idx_pq * 4 + 3]);
  ...
  std::cout << "bad/total: " << bad_points << "/" << key_points.size()
            << std::endl;
}

void OrbMy::BfMatch(const std::vector<DescType> &desc1,
