...
bool ProjectionTdFactor::Evaluate(double const *const *parameters,
  ...
  // td - td_i
  // 帧i相对于imu的时间偏移，乘以速度得到像素差，pts_i减去该部分，得到偏移前的像素坐标
  pts_i_td = pts_i - (td - td_i + TR / ROW * row_i) * velocity_i;
  pts_j_td = pts_j - (td - td_j + TR / ROW * row_j) * velocity_j;

  Eigen::Vector3d pts_camera_i =
      pts_i_td / inv_dep_i; // 归一化相机坐标转相机坐标
  ...
  Eigen::Vector3d pts_imu_j =
      Qj.inverse() * (pts_w - Pj); // 将该特征点从世界坐标系转到j帧imu坐标系
  Eigen::Vector3d pts_camera_j =
      qic.inverse() * (pts_imu_j - tic); // 转到j帧相机坐标
